...
echo "Pre-training"
torchrun_srun -m scripts.base_train -- --depth=32 --target-param-data-ratio=20 --device-batch-size=4 --save-every=10000 --resume-from-step=-1 --run="$WANDB_RUN"
torchrun_srun -m scripts.base_loss
torchrun_srun -m scripts.base_eval

# midtrain
torchrun_srun -m scripts.mid_train -- --device-batch-size=4 --run="$WANDB_RUN"
torchrun_srun -m scripts.chat_eval -- -i mid

# sft
torchrun_srun -m scripts.chat_sft -- --run="$WANDB_RUN"
torchrun_srun -m scripts.chat_eval -- -i sft
