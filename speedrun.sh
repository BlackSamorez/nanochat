...
# torchrun --rdzv_backend=static --rdzv_id=speedrun --nproc_per_node=$NPROC_PER_NODE -m scripts.mid_train -- --run=$WANDB_RUN --device-batch-size=16 --model-tag="$QAT_METHOD"
# torchrun --rdzv_backend=static --rdzv_id=speedrun --nproc_per_node=$NPROC_PER_NODE -m scripts.chat_eval -- -i mid --model-tag="$QAT_METHOD"

# -----------------------------------------------------------------------------
# Supervised Finetuning (domain adaptation to each sequence all by itself per row)

# train sft and re-eval right away (should see a small bump)
# torchrun --rdzv_backend=static --rdzv_id=speedrun --nproc_per_node=$NPROC_PER_NODE -m scripts.chat_sft -- --run=$WANDB_RUN --device-batch-size=4 --model-tag="$QAT_METHOD"
# torchrun --rdzv_backend=static --rdzv_id=speedrun --nproc_per_node=$NPROC_PER_NODE -m scripts.chat_eval -- -i sft --model-tag="$QAT_METHOD"
...
# torchrun --rdzv_backend=static --rdzv_id=speedrun --nproc_per_node=$NPROC_PER_NODE -m scripts.chat_rl -- --run=$WANDB_RUN
# eval the RL model only on GSM8K
# torchrun --rdzv_backend=static --rdzv_id=speedrun --nproc_per_node=$NPROC_PER_NODE -m scripts.chat_eval -- -i rl -a GSM8K
