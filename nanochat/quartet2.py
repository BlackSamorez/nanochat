...
class Quartet_II_fn(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.amax_storage.e_ht_amax is None or not ctx.delayed_amax:
            ctx.amax_storage.e_ht_amax = (grad_output.reshape(-1, Quartet_II_fn.hadamard_matrix.size(0)) @ Quartet_II_fn.hadamard_matrix.T.to(grad_output.dtype)).abs().max().float()
        ...
        if ctx.amax_storage.weght_tht_amax is None or not ctx.delayed_amax:
            ctx.amax_storage.weght_tht_amax = (weight_fp4.T.reshape(-1, Quartet_II_fn.hadamard_matrix.size(0)) @ Quartet_II_fn.hadamard_matrix.T.to(weight_fp4.dtype)).abs().max().float()
        ...
        if ctx.amax_storage.e_tht_amax is None or not ctx.delayed_amax:
            ctx.amax_storage.e_tht_amax = (grad_output.T.reshape(-1, Quartet_II_fn.hadamard_matrix.size(0)) @ Quartet_II_fn.hadamard_matrix.T.to(grad_output.dtype)).abs().max().float()
        ...
        if ctx.amax_storage.input_tht_amax is None or not ctx.delayed_amax:
            ctx.amax_storage.input_tht_amax = (input_fp4.T.reshape(-1, Quartet_II_fn.hadamard_matrix.size(0)) @ Quartet_II_fn.hadamard_matrix.T.to(input_fp4.dtype)).abs().max().float()
...
class Quartet_II_Linear(torch.nn.Linear):
    def __init__(self, *args, hadamard_dim=128, delayed_amax=False, disable_forward_quant=False, disable_backward_quant=False, four_over_six=True, **kwargs):
        ...
        if Quartet_II_fn.hadamard_matrix is None:
            Quartet_II_fn.hadamard_matrix = get_hadamard_matrix(self.hadamard_dim, device="cuda", dtype=torch.bfloat16)
