...
export WANDB_RUN="1b-${QAT_METHOD}-seed${MIXING_SEED}"

# Logs dir
mkdir -p "/iopsstor/scratch/cscs/blacksamorez/nanochat/logs/${QAT_METHOD}/${MIXING_SEED}"
...
torchrun_srun () {
    ...
    srun \
      --kill-on-bad-exit=1 \
      --export=ALL \
      --output="/iopsstor/scratch/cscs/blacksamorez/nanochat/logs/${QAT_METHOD}/${MIXING_SEED}/%x_%j_%s_t%t.out" \
      --error="/iopsstor/scratch/cscs/blacksamorez/nanochat/logs/${QAT_METHOD}/${MIXING_SEED}/%x_%j_%s_t%t.err" \
      --label \
      bash -lc "$CMD"
}
...
echo "Pre-training"
torchrun_srun -m scripts.base_train -- --depth=28 --target-param-data-ratio=20 --device-batch-size=4 --save-every=2000 --resume-from-step=62000 --run="$WANDB_RUN" --model-tag="${QAT_METHOD}_1b"
torchrun_srun -m scripts.base_loss -- --model-tag="${QAT_METHOD}_1b"
torchrun_srun -m scripts.base_eval --  --model-tag="${QAT_METHOD}_1b"

# midtrain
# torchrun_srun -m scripts.mid_train -- --device-batch-size=4 --run="$WANDB_RUN" --model-tag="${QAT_METHOD}_${MIXING_SEED}"
# torchrun_srun -m scripts.chat_eval -- -i mid --model-tag="${QAT_METHOD}_${MIXING_SEED}"

# sft
# torchrun_srun -m scripts.chat_sft -- --run="$WANDB_RUN" --model-tag="${QAT_METHOD}_${MIXING_SEED}" --device-batch-size=1
# torchrun_srun -m scripts.chat_eval -- -i sft --model-tag="${QAT_METHOD}_${MIXING_SEED}"
